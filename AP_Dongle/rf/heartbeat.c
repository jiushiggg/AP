...
#pragma pack()
    ...
    esl_regist_buf.info.battery = gelectric_quantity; //bit[2:0]//电池电量(adc_voltage())
    esl_regist_buf.info.flash_err = gflash_empty; //bit[7]//外部flash异常标志

    esl_regist_buf.info.encryption_enable = aes128.flag.aes_enable; //bit[5] 是否支持加密
    esl_regist_buf.info.isencryption = aes128.flag.isencryption; //bit[6] 是否已经加密
...
#pragma pack()
    ...
    if((screen_dis_t.src_color == BLACK_WHITE_RED)||(screen_dis_t.src_color == BLACK_WHITE_YELLOW))
    {
